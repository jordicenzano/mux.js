...
var
  ...
  parseH264NALs = function (tag, obj, options) {
    const nalTypes = [
      'Unspecified',
    ...
    const nalByteStream = new codecs.h264.NalByteStream(optionsNalParser);
    obj.nalUnits = [];

    let offset = 0
    nalByteStream.on('data', function(data) {
      const nalTypeNum = data[0] & parseInt("00011111",2);
      const nalObj = {nalTypeNum: nalTypeNum, nalType: nalTypes[nalTypeNum]};
      nalObj.dataSize = data.length;
      nalObj.dataOffset = offset;
      offset += AVCHeaderLength + data.length
      if (options.passNALUsDataBuffer === true) {
        nalObj.data = data;
      }
      obj.nalUnits.push(nalObj);
    });
    nalByteStream.push({ data: new Uint8Array(tag)});
  },
  ...
  parseAVCTag = function(tag, obj, options) {
    ...
    if (tag[0] === 1) {
      if (options.parseNALunits === true) {
        parseH264NALs(tag.subarray(4), obj, options)
      }
